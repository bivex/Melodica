...
class AutoPumper:
    ...
    """
    def __init__(self, bpm: float, depth: float = 1.0, shape: float = 2.0, sample_rate: int = 44100):
        """
        Parameters
        ----------
        bpm : float
...
class HardClipper:
    ...
    def process(self, audio: np.ndarray) -> np.ndarray:
        ...
        clipped = np.clip(audio, -self.threshold_linear, self.threshold_linear)
        return clipped * self.makeup_linear
...
class MultibandSaturator:
    ...
    """
    def __init__(self, crossover_hz: float = 150.0, drive_db: float = 12.0, sample_rate: int = 44100):
