...
| `GuideToneGenerator` | `guide_tone.py` | `voice="both"`, voices: 3rd, 7th, both, alternate. `rhythm_value=4.0`, `connect=True`, `velocity_profile="accent_changes"` |
| `EnclosureGenerator` | `enclosure.py` | `enclosure_type="chromatic_above_below"`, types: chromatic_above_below, chromatic_below_above, diatonic_above_below, double_chromatic, delayed, mixed. `target="chord_tones"`, `density=0.7`, `rhythm_placement="mixed"` |
| `SideSlippingGenerator` | `side_slipping.py` | `slip_direction="both"`, directions: up, down, both. `resolution_style="chromatic"`, resolutions: direct, chromatic, scale. `pattern_source="arpeggio"`, `phrase_length=4` |
| `TradingFoursGenerator` | `trading_fours.py` | `trade_length=4`, lengths: 2, 4, 8. `style="call_response"`, styles: call_response, independent, escalating. `player_a_range=(60,84)`, `player_b_range=(48,72)`, `density=1.0` |
| `StopTimeGenerator` | `stop_time.py` | `pattern="big_four"`, patterns: big_four, half_note, syncopated, shuffle, free. `accent_note="chord_tone"`, notes: root, chord_tone, shell. `fill_last_beat=True` |
| `WalkingBassLineGenerator` | `walking_bass_line.py` | `contour="mixed"`, contours: ascending, descending, scalar, arpeggiated, mixed. `target_note="root"`, targets: root, guide_tones, fifths, mixed. `passing_tones="mixed"`, `phrase_length=4` |
| `ShellVoicingGenerator` | `shell_voicing.py` | `voicing_type="root_shell"`, types: root_shell, rootless, spread, A_form, B_form. `rhythm="charleston"`, rhythms: whole_note, half_note, charleston, syncopated, freddie_green. `voice_leading=True`, `drop_2=False`, `include_extensions=False` |
