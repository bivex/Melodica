# Copyright (c) 2026 Bivex
#
# Author: Bivex
# Available for contact via email: support@b-b.top
# For up-to-date contact information:
# https://github.com/bivex
#
# Created: 2026-05-22
# Last Updated: 2026-05-22
#
# Licensed under the MIT License.
# Commercial licensing available upon request.

"""
album_elena.py — "Elena EP".

A beautiful, high-fidelity four-movement modern electronic and acoustic EP
utilizing our advanced 2025 modern bass generators and expressive solo melody generators.
Scale: E Aeolian (E=4, F#=6, G=7, A=9, B=11, C=0, D=2) & E Dorian (E=4, F#=6, G=7, A=9, B=11, C#=1, D=2).
Root: E (4) — resonant, extremely rich register for guitar leads and modern basses.

Movements:
  I.   Dawn of Elena (Рассвет Елены)     — Neo Soul / Alt-R&B in E Aeolian (fingerstyle bass + neo_soul_keys solo)
  II.  Golden Ribbon (Золотая лента)      — Syncopated Future Funk in E Dorian (slap bass + vocal_mimic solo)
  III. Cosmic Horizon (Космический горизонт)— Cosmic Synthwave in E Aeolian (sidechain bass + modal_ambient solo)
  IV.  Amethyst Flight (Аметистовый полет) — Evolving Cinematic in E Dorian (self_modifying bass + cinematic_strings solo)
"""

import random
from pathlib import Path

from melodica import types
from melodica.types import NoteInfo, Scale, Mode, ChordLabel, Quality
from melodica.generators import GeneratorParams
from melodica.generators.melody import MelodyGenerator
from melodica.generators.modern_bass_2025 import ModernBass2025Generator
from melodica.generators.solo_melody import SoloMelodyGenerator
from melodica.midi import export_multitrack_midi
from melodica.shorts_mixing import MixingDesk
from melodica.shorts_mastering import MasteringDesk
from melodica.composer.transformers import spiceup, serialize_canon, Identity, OneToThree
from melodica.harmonize.predictive import PredictiveHarmonizer
from melodica.rhythm.groove_template import SWING_60, LAID_BACK

# E Aeolian (E, F#, G, A, B, C, D)
KEY_AEOLIAN = Scale(root=4, mode=Mode.AEOLIAN)
# E Dorian (E, F#, G, A, B, C#, D)
KEY_DORIAN = Scale(root=4, mode=Mode.DORIAN)


def _build_chords(progression: str, duration: float, key: Scale) -> list[ChordLabel]:
    """Parse a space-separated Roman numeral progression into ChordLabels."""
    parts = progression.split()
    beats_per = duration / len(parts)
    chords = []
    for i, p in enumerate(parts):
        chord = key.parse_roman(p)
        chord.start = i * beats_per
        chord.duration = beats_per
        chords.append(chord)
    return chords


# ---------------------------------------------------------------------------
# I. Dawn of Elena (Рассвет Елены)
# ---------------------------------------------------------------------------

def produce_track_1():
    """Warm Neo-Soul / Alt-R&B in E Aeolian. Fingerstyle bass + neo_soul_keys Rhodes solo."""
    print("Producing I. Dawn of Elena (Neo-Soul / Fingerstyle Bass + Rhodes Solo)...")

    duration = 96.0
    # i - iv - VII - v in E Aeolian
    progression = "i iv VII v" * 6
    chords = _build_chords(progression, duration, KEY_AEOLIAN)

    # Solo: Rhodes single-note lines with offbeat backing stabs
    solo_params = GeneratorParams(density=0.42, key_range_low=52, key_range_high=76)
    solo_gen = SoloMelodyGenerator(solo_params, style="neo_soul_keys", vibrato_depth=0.7)
    raw_solo = solo_gen.render(chords, KEY_AEOLIAN, duration)

    # Refine chords and add light spice
    harmonizer = PredictiveHarmonizer(certainty_threshold=2.0)
    chords = harmonizer.refine(chords, raw_solo, KEY_AEOLIAN, duration)
    solo = spiceup(raw_solo, KEY_AEOLIAN, depth=1)

    # Modern 2025 Legato Fingerstyle Bass (pocket lock: Avg 63, Max 84)
    bass_params = GeneratorParams(density=0.55, key_range_low=32, key_range_high=52)
    bass_gen = ModernBass2025Generator(bass_params, style="fingerstyle")
    bass = bass_gen.render(chords, KEY_AEOLIAN, duration)

    # Warm sustained pad
    pad = [
        NoteInfo(pitch=c.root + 36, start=c.start, duration=c.duration * 1.05, velocity=48)
        for c in chords
    ]

    return {"piano": solo, "bass": bass, "pad": pad}, 80.0


# ---------------------------------------------------------------------------
# II. Golden Ribbon (Золотая лента)
# ---------------------------------------------------------------------------

def produce_track_2():
    """Future Funk / Alt-Pop. Slap bass + vocal_mimic human lead solo in E Dorian."""
    print("Producing II. Golden Ribbon (Future Funk / Slap Bass + Vocal-Mimic Solo)...")

    duration = 120.0
    # i - iv - bVI - V progression (bright major 6th from Dorian makes this feel amazing)
    progression = "i iv bVI V" * 8
    chords = _build_chords(progression, duration, KEY_DORIAN)

    # Solo: Vocal mimic lead with melisma grace-note sweeps
    solo_params = GeneratorParams(density=0.6, key_range_low=55, key_range_high=80)
    solo_gen = SoloMelodyGenerator(solo_params, style="vocal_mimic", vibrato_depth=0.8)
    raw_solo = solo_gen.render(chords, KEY_DORIAN, duration)
    solo = spiceup(raw_solo, KEY_DORIAN, depth=1)

    # Modern 2025 Slap Bass (peaks at 95, classic funk, avg 64)
    bass_params = GeneratorParams(density=0.75, key_range_low=36, key_range_high=62)
    bass_gen = ModernBass2025Generator(bass_params, style="slap")
    bass = bass_gen.render(chords, KEY_DORIAN, duration)

    # High perk accents on strong note peaks
    accents = [
        NoteInfo(pitch=n.pitch + 12, start=n.start, duration=0.15, velocity=105)
        for n in solo
        if n.velocity > 85
    ]

    return {"lead": solo, "bass": bass, "accents": accents}, 118.0


# ---------------------------------------------------------------------------
# III. Cosmic Horizon (Космический горизонт)
# ---------------------------------------------------------------------------

def produce_track_3():
    """Atmospheric Synthwave / Dark Disco in E Aeolian. Sidechain bass + modal_ambient solo."""
    print("Producing III. Cosmic Horizon (Synthwave / Pumping Bass + Detuned Ambient Solo)...")

    duration = 112.0
    # i - bVII - iv - i in E Aeolian
    progression = "i bVII iv i" * 7
    chords = _build_chords(progression, duration, KEY_AEOLIAN)

    # Solo: Modal ambient drone sweeps with microtonal analog VCO detune LFO
    solo_params = GeneratorParams(density=0.35, key_range_low=58, key_range_high=86)
    solo_gen = SoloMelodyGenerator(solo_params, style="modal_ambient", vibrato_depth=0.6)
    raw_solo = solo_gen.render(chords, KEY_AEOLIAN, duration)
    solo = spiceup(raw_solo, KEY_AEOLIAN, depth=1)

    # Rapid 16th-note arpeggiator backing chords
    arp_params = GeneratorParams(density=0.8, key_range_low=52, key_range_high=76)
    arp_gen = MelodyGenerator(arp_params, drama_shape="epic", phrase_length=4.0, beats_per_bar=4)
    arp_raw = arp_gen.render(chords, KEY_AEOLIAN, duration)
    arp = spiceup(arp_raw, KEY_AEOLIAN, depth=1, single_pool=[OneToThree])

    # Modern 2025 Sidechain-Reactive Synth Bass (pumping volume CC 7, Avg 60, Max 84)
    bass_params = GeneratorParams(density=0.7, key_range_low=32, key_range_high=48)
    bass_gen = ModernBass2025Generator(bass_params, style="sidechain_reactive")
    bass = bass_gen.render(chords, KEY_AEOLIAN, duration)

    # Sweeping string pad
    pad = [
        NoteInfo(pitch=c.root + 48, start=c.start, duration=c.duration * 1.1, velocity=40)
        for c in chords
    ]

    return {"lead": solo, "arp": arp, "bass": bass, "pad": pad}, 124.0


# ---------------------------------------------------------------------------
# IV. Amethyst Flight (Аметистовый полет)
# ---------------------------------------------------------------------------

def produce_track_4():
    """Orchestral Ambient Rise & Release in E Dorian. Self-modifying bass + cinematic_strings solo."""
    print("Producing IV. Amethyst Flight (Cinematic Ambient / Evolving Bass + String Solo)...")

    duration = 128.0
    # Long drone progression: i - v - iv - i (4 bars per chord = 16 beats)
    progression = "i i v v iv iv i i" * 2
    chords = _build_chords(progression, duration, KEY_DORIAN)

    # Solo: Cinematic string solo with tension b9/#11 resolution and CC 1 tremolos
    solo_params = GeneratorParams(density=0.36, key_range_low=52, key_range_high=82)
    solo_gen = SoloMelodyGenerator(solo_params, style="cinematic_strings", vibrato_depth=0.9)
    raw_solo = solo_gen.render(chords, KEY_DORIAN, duration)
    solo = spiceup(raw_solo, KEY_DORIAN, depth=1)

    # Canon for massive orchestration depth
    canon = serialize_canon(
        voices=[solo, solo],
        delay_beats=8.0,
        transpositions=[0, -12],
        duration_beats=duration,
    )

    # Modern 2025 Self-Modifying Bass (widens ranges/syncopation dynamically, Avg 66, Max 90)
    bass_params = GeneratorParams(density=0.6, key_range_low=24, key_range_high=48)
    bass_gen = ModernBass2025Generator(bass_params, style="self_modifying")
    bass = bass_gen.render(chords, KEY_DORIAN, duration)

    # Deep brass swells on transitions
    brass = [
        NoteInfo(pitch=c.root + 36, start=c.start, duration=2.5, velocity=90)
        for c in chords
    ]

    return {"canon_lead": canon, "bass": bass, "brass": brass}, 88.0


# ---------------------------------------------------------------------------
# Post-production
# ---------------------------------------------------------------------------

def apply_post_production(raw_tracks, bpm, lufs=-14.0):
    desk = MixingDesk(niche_cfg={})
    desk.track_gains.update({
        "piano": 0.95,
        "lead": 0.90,
        "arp": 0.80,
        "canon_lead": 0.85,
        "bass": 1.15,  # Focus modern basses forward
        "pad": 0.48,
        "accents": 0.92,
        "brass": 0.85,
    })

    mixed = desk.apply_mixing(raw_tracks, [], int(bpm))
    master = MasteringDesk(target_lufs=lufs)
    mastered, pan_events = master.apply_mastering(mixed)
    return mastered, pan_events


# ---------------------------------------------------------------------------
# Main
# ---------------------------------------------------------------------------

def main():
    album_dir = Path("output/album_elena")
    album_dir.mkdir(exist_ok=True, parents=True)

    print("\n" + "=" * 60)
    print("   ELENA EP — Album in E Aeolian & E Dorian")
    print("   Scale: E F# G A B C D / E F# G A B C# D")
    print("   Featuring Expressive Solo & Bass Generators")
    print("=" * 60 + "\n")

    # I. Dawn of Elena
    t1_raw, t1_bpm = produce_track_1()
    t1_m, t1_pan = apply_post_production(t1_raw, t1_bpm, lufs=-16.0)
    export_multitrack_midi(
        t1_m, str(album_dir / "01_Dawn_of_Elena.mid"),
        bpm=t1_bpm, cc_events=t1_pan,
        instruments={"piano": 5, "bass": 34, "pad": 89},
    )

    # II. Golden Ribbon
    t2_raw, t2_bpm = produce_track_2()
    t2_m, t2_pan = apply_post_production(t2_raw, t2_bpm, lufs=-12.0)
    export_multitrack_midi(
        t2_m, str(album_dir / "02_Golden_Ribbon.mid"),
        bpm=t2_bpm, cc_events=t2_pan,
        instruments={"lead": 81, "bass": 37, "accents": 14},
    )

    # III. Cosmic Horizon
    t3_raw, t3_bpm = produce_track_3()
    t3_m, t3_pan = apply_post_production(t3_raw, t3_bpm, lufs=-13.0)
    export_multitrack_midi(
        t3_m, str(album_dir / "03_Cosmic_Horizon.mid"),
        bpm=t3_bpm, cc_events=t3_pan,
        instruments={"lead": 82, "arp": 81, "bass": 39, "pad": 89},
    )

    # IV. Amethyst Flight
    t4_raw, t4_bpm = produce_track_4()
    t4_m, t4_pan = apply_post_production(t4_raw, t4_bpm, lufs=-14.0)
    export_multitrack_midi(
        t4_m, str(album_dir / "04_Amethyst_Flight.mid"),
        bpm=t4_bpm, cc_events=t4_pan,
        instruments={"canon_lead": 41, "bass": 39, "brass": 61}, # violin string lead (41)
    )

    print("\n" + "=" * 60)
    print("   PRODUCTION COMPLETE: ELENA EP")
    print(f"   MIDI output saved under: {album_dir.resolve()}")
    print("=" * 60 + "\n")


if __name__ == "__main__":
    main()
