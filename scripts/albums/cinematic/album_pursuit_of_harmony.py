...
"""
scripts/albums/cinematic/album_pursuit_of_harmony.py — "Погоня за гармонией"
(Pursuit of Harmony): a 7-movement cinematic suite tracing a search for harmonic
resolution across the modal-brightness axis. Each movement steps from the most
unstable mode (Locrian, diminished tonic — harmony perpetually out of reach) to
the most consonant (Ionian — harmony finally found). Cadences tighten as the
suite advances: avoidance → Phrygian descent → exotic detour → deceptive
resolution → Dorian lift → Mixolydian openness → definitive I–IV–V–I.
...
def generate_pursuit_album():
    album_dir = Path("output/album_pursuit_of_harmony")
    album_dir.mkdir(exist_ok=True, parents=True)

    print()
    print("=" * 80)
    print("        П О Г О Н Я   З А   Г А Р М О Н И Е Й")
    ...
    print("\n" + "=" * 80)
    print(f"  ALBUM COMPLETE: Pursuit of Harmony")
