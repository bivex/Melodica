...
def _lead_melody(scale, dur, *, lo, hi, density):
    ...
    gen = MelodyGenerator(p, phrase_length=8.0,
                          ...
                          register_smoothness=0.82, steps_probability=0.78,
...
def _mix(raw, bpm, lufs=-14.0):
    ...
    desk.track_gains.update({
        "Lead": 0.84, "Drone": 0.82, "Drone2": 0.78, "Violin1": 0.78,
        "Viola": 0.74, "Cello": 0.80, "Bass": 0.84, "Strings": 0.76,
        "Tremolo": 0.74, "Brass": 0.78, "Choir": 0.80, "Harp": 0.78,
        "Oboe": 0.74, "Flute": 0.74, "Clarinet": 0.74, "Bassoon": 0.76,
        "Counter": 0.72, "Ostinato": 0.74, "Glock": 0.70, "Mallet": 0.72,
        "Timpani": 0.80, "Bells": 0.76,
    })
...
def track_01_sikah():
    ...
    # A: tonic - 6th - 2nd - tonic  (the Sikah salām via the 2nd)
    # B: tonic - 3rd - 2nd - tonic
    phrases = [
        [(0, 1), (5, 1), (1, 1), (0, 1)],
        [(0, 1), (2, 1), (1, 1), (0, 1)],
    ]
    ...
    drone = DroneGenerator(GeneratorParams(density=0.1, velocity_range=(55, 68)),
                           variant="power", fade_in=4.0).render(chords, key, dur)
    drone_hi = _clamp(DroneGenerator(
    ...
    ).render(chords, key, dur), 40, 76), dur, intro_end=dur*0.30, outro_start=dur*0.75, keep=0.15)
    ...
    return {
        "Lead": _clamp(lead, 50, 88), "Drone": drone, "Drone2": drone_hi,
        "Strings": strings, "Oboe": oboe, "Cello": cello, "Counter": counter, "Bass": bass,
    }, bpm
...
def track_02_huzam():
    ...
    clarinet = _thin(_clamp(ClarinetGenerator(
        GeneratorParams(density=0.38, key_range_low=55, key_range_high=79),
        articulation="legato").render(chords, key, dur), 40, 76), dur)
    ...
    return {
        "Lead": _clamp(lead, 50, 86), "Drone": drone, "Violin1": violin,
        "Strings": strings, "Choir": choir, "Clarinet": clarinet, "Bass": bass,
    }, bpm
...
def track_03_baladi():
    ...
    mallet = _thin(_clamp(MalletPercussionGenerator(
        ...
        instrument="marimba", pattern="run").render(chords, key, dur), 44, 82), dur)

    strings = _thin(_clamp(StringsLegatoGenerator(
        GeneratorParams(density=0.28, key_range_low=50, key_range_high=68),
        section_size="full", dynamic_shape="crescendo").render(chords, key, dur), 38, 74), dur,
        intro_end=dur*0.25, outro_start=dur*0.78, keep=0.15)

    bass = _clamp(ContrabassGenerator(
        GeneratorParams(density=0.75, key_range_low=26, key_range_high=40),
        articulation="pizzicato").render(chords, key, dur), 48, 86)
    ...
    return {
        "Lead": _clamp(lead, 55, 92), "Drone": drone, "Ostinato": ostinato,
        "Violin1": violin, "Flute": flute, "Mallet": mallet,
        "Strings": strings, "Bass": bass, "Timpani": timp,
    }, bpm
...
def track_04_iraq():
    ...
    drone = DroneGenerator(GeneratorParams(density=0.1, velocity_range=(55, 68)),
                           variant="power", fade_in=5.0).render(chords, key, dur)
    drone_hi = _clamp(DroneGenerator(
        GeneratorParams(density=0.1, velocity_range=(45, 58)),
        variant="octave", fade_in=7.0).render(chords, key, dur), 40, 60)

    strings = _thin(_clamp(StringsLegatoGenerator(
        GeneratorParams(density=0.28, key_range_low=48, key_range_high=68),
        section_size="full", dynamic_shape="cresc_dim").render(chords, key, dur), 35, 66), dur)

    tremolo = _thin(_clamp(TremoloStringsGenerator(
        GeneratorParams(density=0.25, key_range_low=48, key_range_high=62),
        variant="single", bow_speed=0.20).render(chords, key, dur), 34, 62), dur)
    ...
    harp = _thin(_clamp(HarpGenerator(
        GeneratorParams(density=0.2, key_range_low=48, key_range_high=80),
        pattern="arpeggio", direction="up_down", octave_span=3).render(chords, key, dur), 38, 72), dur)

    bass = _clamp(ContrabassGenerator(
        GeneratorParams(density=0.5, key_range_low=24, key_range_high=36),
        articulation="legato").render(chords, key, dur), 44, 74)

    return {
        "Lead": _clamp(lead, 50, 86), "Drone": drone, "Drone2": drone_hi,
        "Strings": strings, "Tremolo": tremolo, "Bassoon": bassoon,
        "Choir": choir, "Harp": harp, "Bass": bass,
    }, bpm
...
def track_05_return():
    ...
    drone = DroneGenerator(GeneratorParams(density=0.1, velocity_range=(55, 70)),
                           variant="power", fade_in=4.0).render(chords, key, dur)

    brass = _thin(_clamp(BrassSectionGenerator(
        GeneratorParams(density=0.35, key_range_low=48, key_range_high=72),
        ensemble_mode="full", intensity=0.85).render(chords, key, dur - 6.0), 48, 92), dur)

    violin = _thin(_clamp(ViolinGenerator(
        GeneratorParams(density=0.5, key_range_low=67, key_range_high=91),
        articulation="legato").render(chords, key, dur), 48, 92), dur)

    strings = _thin(_clamp(StringsLegatoGenerator(
        GeneratorParams(density=0.35, key_range_low=50, key_range_high=72),
        section_size="full", dynamic_shape="crescendo").render(chords, key, dur), 42, 82), dur)

    choir = _thin(_clamp(ChoirAahsGenerator(
        GeneratorParams(density=0.32, key_range_low=50, key_range_high=74),
        voice_count=6, dynamics="f", syllable="aah").render(chords, key, dur), 42, 86), dur)

    oboe = _clamp(OboeGenerator(
        GeneratorParams(density=0.4, key_range_low=67, key_range_high=88),
        articulation="legato", register=2).render(chords, key, dur - 8.0), 42, 82)
    oboe = _thin(_off(oboe, 8.0), dur)

    harp = _thin(_clamp(HarpGenerator(
        GeneratorParams(density=0.45, key_range_low=48, key_range_high=84),
        pattern="arpeggio", direction="up_down").render(chords, key, dur), 40, 80), dur)

    bass = _clamp(ContrabassGenerator(
        GeneratorParams(density=0.7, key_range_low=24, key_range_high=40),
        articulation="legato").render(chords, key, dur), 48, 86)
    ...
    return {
        "Lead": _clamp(lead, 52, 92), "Drone": drone, "Brass": brass,
        "Violin1": violin, "Strings": strings, "Choir": choir, "Oboe": oboe,
        "Harp": harp, "Bass": bass, "Timpani": timp,
    }, bpm
...
TRACKS = [
    (track_01_sikah, "01_Sikah.mid", {
        "Lead": 68, "Drone": 48, "Drone2": 48, "Strings": 48,
        "Oboe": 68, "Cello": 42, "Counter": 40, "Bass": 43,
    }),
    (track_02_huzam, "02_Huzam.mid", {
        "Lead": 71, "Drone": 48, "Violin1": 40, "Strings": 48,
        "Choir": 52, "Clarinet": 71, "Bass": 43,
    }),
    (track_03_baladi, "03_Sikah_Baladi.mid", {
        "Lead": 68, "Drone": 48, "Ostinato": 45, "Violin1": 40,
        "Flute": 73, "Mallet": 13, "Strings": 48, "Bass": 43, "Timpani": 47,
    }),
    (track_04_iraq, "04_Iraq.mid", {
        "Lead": 68, "Drone": 48, "Drone2": 48, "Strings": 48,
        "Tremolo": 44, "Bassoon": 70, "Choir": 52, "Harp": 46, "Bass": 43,
    }),
    (track_05_return, "05_Sikah_Return.mid", {
        "Lead": 68, "Drone": 48, "Brass": 61, "Violin1": 40,
        "Strings": 48, "Choir": 52, "Oboe": 68, "Harp": 46, "Bass": 43, "Timpani": 47,
    }),
]
