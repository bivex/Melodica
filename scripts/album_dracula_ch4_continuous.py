...
def build_secret_correspondence() -> dict:
    ...
    REFACTORED V3: Psychoacoustic phrasing, call-response, and episodic bass."""
    bpm, dur = 72, 96.0
    chords = _loop_chords(types.parse_progression("i:4.0 - iv:4.0 - V:4.0 - i:4.0", KEY_MINOR), dur)

    # 1. BASS - Episodic 'Pedal Drops' (BOOMs instead of carpet)
    bass = BassGenerator(
        GeneratorParams(density=0.15, velocity_range=(65, 85), key_range_low=23, key_range_high=35),
        style="walking"
    ).render(chords, KEY_MINOR, dur)
    # Ensure bass only plays on beat 1 of every 2 bars (Pedal drops)
    bass = [n for n in bass if (n.start % 8.0) < 0.5]

    # 2. GUITAR - Harmonic Body (Articulation Shift: Sustain -> Arp)
    guitar_notes = ArpeggiatorGenerator(
        GeneratorParams(density=0.45, velocity_range=(55, 75), key_range_low=55, key_range_high=75),
        pattern="up_down"
    ).render(chords, KEY_MINOR, dur)
    # Dynamic subtraction: guitar disappears in the middle section
    guitar_notes = [n for n in guitar_notes if not (48.0 <= n.start < 64.0)]

    # 3. PAD - Airy layer (Distant/Spatial focus)
    pad = AmbientPadGenerator(
        GeneratorParams(density=0.08, velocity_range=(35, 50), key_range_low=72, key_range_high=96),
        voicing="spread"
    ).render(chords, KEY_MINOR, dur)
    # Staggered entry and exit
    pad = [n for n in pad if 16.0 <= n.start < 80.0]

    # 4. VIOLIN - Fragmented Phrasing (Call-Response / Invocations)
    violin_notes = MelodyGenerator(
        GeneratorParams(density=0.35, complexity=0.75, velocity_range=(70, 100),
                        key_range_low=67, key_range_high=93),
        phrase_length=4.0, motif_probability=0.90
    ).render(chords, KEY_MINOR, dur)
    # Orchestral breathing: remove notes to create fragments
    # Logic: 4 beats of play, 4 beats of silence (Response gap)
    violin_notes = [n for n in violin_notes if (int(n.start // 4.0) % 2 == 0)]

    cc_events = {
        "violin": AutomationCurve.exponential(11, 40, 105, 0.0, dur, exponent=1.2, steps=40),
        "pad":    AutomationCurve.sine_lfo(74, 20, 80, 16.0, 80.0, period=20.0),
    }

    return {
        "tracks":      {"guitar": guitar_notes, "violin": violin_notes, "pad": pad, "bass": bass},
        "bpm":         bpm,
...
def build_draculas_vault() -> dict:
    ...
    REFACTORED V2: Spectral air (High choir), asymmetric rhythm, and better spread."""
    bpm, dur = 50, 80.0
    
    # Asymmetric harmonic rhythm: 6 + 2 + 5 + 3 instead of 4+4
    chords = []
    prog = [
        (11, Quality.MINOR, 6.0),
        (0,  Quality.MAJOR, 2.0),
        (10, Quality.MINOR, 5.0),
        (5,  Quality.MINOR, 3.0)
    ]
    t = 0.0
    while t < dur:
        for root, qual, d in prog:
            if t >= dur: break
            chords.append(types.ChordLabel(root=root, quality=qual, start=t, duration=min(d, dur-t)))
            t += d

    # 1. BASS - Foundation
    contrabass_notes = BassGenerator(
        GeneratorParams(density=0.25, velocity_range=(45, 65), key_range_low=23, key_range_high=35)
    ).render(chords, KEY_PHRYGIAN, dur)

    # 2. MID - Organ (restricted)
    organ = AmbientPadGenerator(
        GeneratorParams(density=0.15, velocity_range=(40, 60), key_range_low=47, key_range_high=59)
    ).render(chords, KEY_PHRYGIAN, dur)

    # 3. HIGH AIR - Spectral Layer (Distant Choir Harmonics)
    choir_air = AmbientPadGenerator(
        GeneratorParams(density=0.08, velocity_range=(35, 55), key_range_low=84, key_range_high=108),
        voicing="cluster"
    ).render(chords, KEY_PHRYGIAN, dur)
    # Silence architecture: choir only appears in sections
    choir_air = [n for n in choir_air if not (32.0 <= n.start < 48.0)]

    # 4. LEAD - Cello (Emotional motion)
    cello_notes = MelodyGenerator(
        GeneratorParams(density=0.35, complexity=0.50, velocity_range=(60, 80),
                        key_range_low=48, key_range_high=72),
        phrase_length=12.0 # Longer, slow phrasing
    ).render(chords, KEY_MINOR, dur)
    ...
    cc_events = {
        "cello":      AutomationCurve.sine_lfo(11, 40, 90, 0.0, dur, period=15.0),
        "organ":      AutomationCurve.linear(74, 20, 60, 0.0, dur, steps=20),
        "choir_air":  AutomationCurve.exponential(7, 30, 80, 0.0, dur, exponent=1.2, steps=20),
    }

    return {
        "tracks":      {"cello": cello_track.notes, "organ": organ, 
                        "contrabass": contrabass_notes, "choir_air": choir_air},
        "bpm":         bpm,
...
def build_shovels_strike() -> dict:
    ...
    REFACTORED V3: Doom foundation, balanced mids, and dynamic deception."""
    bpm, dur = 80, 48.0
    chords = _loop_chords(types.parse_progression("i:2.0 - V:2.0 - VI:2.0 - iv:2.0", KEY_MINOR), dur)

    # 1. BASS - Foundation (Doom Notes)
    # Restore sub-gravity but as events
    contrabass = BassGenerator(
        GeneratorParams(density=0.10, velocity_range=(80, 110), key_range_low=23, key_range_high=35)
    ).render(chords, KEY_MINOR, dur)
    # Contrabass only on important beat 1 changes
    contrabass = [n for n in contrabass if (n.start % 16.0) < 0.5]

    timpani = RhythmicAccentGenerator(
        preset="march", pitch=36, velocity_humanize=8, accent_strength=1.3
    ).render(chords, KEY_MINOR, dur)
    # Dynamic deception: silence right before the end
    timpani = [n for n in timpani if n.start < 40.0]

    # 2. MID-GLUE - Violas and Horns (Body)
    viola = StringsEnsembleGenerator(
        GeneratorParams(density=0.50, velocity_range=(65, 85), key_range_low=48, key_range_high=64),
        articulation="tremolo"
    ).render(chords, KEY_MINOR, dur)
    
    horns = StringsEnsembleGenerator(
        GeneratorParams(density=0.35, velocity_range=(70, 95), key_range_low=47, key_range_high=71)
    ).render(chords, KEY_MINOR, dur)
    # Horns only enter in the second half for 'climax expansion'
    horns = [n for n in horns if n.start >= 24.0]

    # 3. HIGH Anxiety
    violin_notes = MelodyGenerator(
        GeneratorParams(density=0.55, complexity=0.85, velocity_range=(90, 115),
    ...
    sparkle = ArpeggiatorGenerator(
        GeneratorParams(density=0.15, velocity_range=(45, 65), key_range_low=96, key_range_high=115),
        pattern="random", note_duration=0.125
    ).render(chords, KEY_MINOR, dur)

    tempo_events = [(float(b), 80.0 + (140.0 - 80.0) * (b / dur)) for b in range(0, int(dur), 2)]

    cc_events = {
        "violin":  AutomationCurve.exponential(1, 40, 120, 16.0, dur, exponent=1.4, steps=20),
        "viola":   AutomationCurve.linear(11, 40, 100, 0.0, dur, steps=20),
        "horns":   AutomationCurve.exponential(11, 60, 115, 24.0, dur, exponent=1.8, steps=15),
        "sparkle": AutomationCurve.sine_lfo(74, 30, 95, 0.0, dur, period=10.0),
    }

    return {
        "tracks":       {"violin": violin_notes, "viola": viola, "horns": horns,
                        "timpani": timpani, "contrabass": contrabass, "sparkle": sparkle},
        "bpm":          bpm,
        "instruments":  {"violin": 40, "viola": 41, "horns": 60, "timpani": 47, "contrabass": 43, "sparkle": 98},
        "cc_events":    cc_events,
        "tempo_events": tempo_events,
        "key":          KEY_MINOR,
    }
