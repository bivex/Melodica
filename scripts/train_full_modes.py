...
def main():
    ...
    # shifts[r, p] = (p + r) % 12
    shifts = (torch.arange(N_TONES, device=device).view(1, N_TONES) + 
              torch.arange(N_TONES, device=device).view(N_TONES, 1)) % N_TONES
    ...
    for iter_idx in pbar:
        ...
        for t in range(1, max_t):
            ...
            combined_prev = torch.einsum('nrit,tir->nrt', prev_expanded, pchange)
        ...
        for t in range(max_t - 2, -1, -1):
            ...
            combined_next = torch.einsum('nrit,tit->nrt', next_expanded, pchange)
        ...
        change_hist = torch.einsum('ntrip,ntrn->pin', term_prev_expanded, term_next)
