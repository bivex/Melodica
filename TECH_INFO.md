...
### Изоляция MIDI-каналов между треками (Channel Pool Isolation)

Для предотвращения перекрёстных помех (cross-talk) при использовании Pitch Bend (который в спецификации MIDI применяется ко всему каналу целиком, а не к отдельным нотам), функции экспорта реализуют изоляцию пулов каналов:

- **Глобальное распределение**: Всего доступно 16 стандартных MIDI-каналов. Канал 9 (10-й канал в 1-based нумерации) зарезервирован исключительно под ударные (`drums`) и не используется для тональных инструментов.
- **Изолированные пулы треков**: При экспорте многодорожечного MIDI-файла через `export_multitrack_midi()`, каждый трек получает свой собственный, не пересекающийся с другими треками пул MIDI-каналов (обычно по 3 канала на трек):
  - Трек 1: каналы `[0, 1, 2]`
  - Трек 2: каналы `[3, 4, 5]`
  - Трек 3: каналы `[6, 7, 8]`
  - Трек 4 (Ударные): канал `[9]` (фиксированно)
  - Трек 5: каналы `[10, 11, 12]`
  - Трек 6: каналы `[13, 14, 15]`
- **Предотвращение Cross-Talk**: Поскольку полифонические микротональные ноты на одном треке распределяются в рамках его собственного пула каналов, Pitch Bend сообщения одного трека гарантированно не влияют на звучание нот других треков.
